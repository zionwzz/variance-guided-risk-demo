# ============================================================
# lmvt(): Penalized panel ARX-GARCH-X estimator
# ------------------------------------------------------------
# data: columns s, t, y, plus covariate columns (all non s,t,y are X)
# p, q    : mean lags (AR on y; X lags 0..q in the mean)
# r, s_ord: variance lags (ARCH/GARCH on eps^2 and sigma^2)
# lambda_beta : L1 penalty on mean covariates (beta)
# lambda_gamma: L1 penalty on variance covariates (gamma)
# standardize_X: standardize covariates before fitting
# use_x_in_variance: if FALSE, the variance excludes X (gamma = 0)
# ============================================================
...
lmvt <- function(data,
  ...
  if (!use_x_in_variance) {
    return(lmvt_core(data, p, q, r, s_ord, lambda_beta, lambda_gamma,
                     maxit, tol, standardize_X, use_x_in_variance,
                     phi_cap, omega_min, omega_cap_mult,
                     gamma_steps, gamma_step, verbose))
  }

  if (verbose) cat("Estimating variance model with X...\n\n")

  if (verbose) cat("Estimating mean + ARCH/GARCH (no X in variance)...\n")

  base_fit <- lmvt_core(
    data = data, p = p, q = q, r = r, s_ord = s_ord,
    lambda_beta = lambda_beta,
    lambda_gamma = 0,
    use_x_in_variance = FALSE,
  ...
  if (verbose) {
    ...
    cat(sprintf("  theta = %.4f, beta[1] = %.4f\n",
                if(length(base_fit$theta)) base_fit$theta[1] else 0,
                base_fit$beta[1]))
    cat(sprintf("  a = %.4f, b = %.4f, mean(omega) = %.4f\n\n",
                if(length(base_fit$a)) base_fit$a else 0,
                if(length(base_fit$b)) base_fit$b else 0,
                mean(base_fit$omega)))
  }

  if (verbose) cat("Estimating gamma from residual variance...\n")
  ...
  xcols <- base_fit$xcols
  Xraw <- data.matrix(data[, xcols, drop = FALSE])
  if (base_fit$standardize_X) {
    X <- scale(Xraw, center = base_fit$x_center, scale = base_fit$x_scale)
  } else {
    X <- Xraw
  }
  Y <- as.numeric(data$y)
  ...
  AR_part <- if (p > 0) as.numeric(Ylags %*% base_fit$theta) else 0
  X_part <- as.numeric(Xmean %*% base_fit$beta)
  mu_base <- base_fit$alpha[match(data$s, ids)] + AR_part + X_part
  eps <- Y - mu_base
  eps[!is.finite(eps)] <- 0

  sigma2_base <- rep(NA_real_, n)
    ...
    for (ii in seq_along(idx)) {
      ...
      if (r > 0) {
        for (L in 1:r) {
          ...
          if (length(tprev) == 1) {
            arch_part <- arch_part + base_fit$a[L] * (eps[idx[tprev]]^2)
          }
        }
      }
      if (s_ord > 0) {
        for (L in 1:s_ord) {
          tprev <- which(data$s[idx] == id & data$t[idx] == (data$t[tt] - L))
          if (length(tprev) == 1) {
            garch_part <- garch_part + base_fit$b[L] * sigma2_base[idx[tprev]]
          }
        }
      }
      sigma2_base[tt] <- max(base_fit$omega[match(id, ids)] + arch_part + garch_part, 1e-8)
    }
  }

  VL <- make_var_lags(eps, sigma2_base)
  mask_var <- rep(TRUE, n)
  ...
  mask_var <- mask_var & is.finite(eps) & is.finite(sigma2_base)
  valid_idx <- which(mask_var)

  residual_var <- (eps^2 - sigma2_base)[valid_idx]
  Xvar_valid <- Xvar[valid_idx, , drop = FALSE]

  weights <- 1 / pmax(sigma2_base[valid_idx], 1e-6)
  ...
  structure(list(
    ...
    theta = base_fit$theta,
    beta = base_fit$beta,
    omega = base_fit$omega,
    a = base_fit$a,
    b = base_fit$b,
    gamma = gamma,
    xcols = xcols,
    standardize_X = base_fit$standardize_X,
    use_x_in_variance = TRUE,
    x_center = base_fit$x_center,
    x_scale = base_fit$x_scale,
...
# ============================================================
# Core fit used by lmvt()
# ============================================================
lmvt_core <- function(data,
                 p = 1, q = 0,
                 r = 1, s_ord = 1,
                 lambda_beta = 0, lambda_gamma = 0,
                 maxit = 200, tol = 1e-4,
