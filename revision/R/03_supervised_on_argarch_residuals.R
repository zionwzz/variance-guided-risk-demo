...
#   "how the supervised comparators perform if they are used to model the
#    residuals of AR-GARCH, though the improvement may be limited since the
#    AUC is already high with the temporal dependence ignored."
#
# Design: fit a panel AR-GARCH to HR (no covariates), form the standardized
# residual  z = (y - muhat)/sigmahat  (temporal mean+variance dependence
# removed), then run the SAME supervised comparators (logistic, RF, GBM) under
# leave-one-subject-out CV in two ways:
#   (baseline)         77 raw window features
#   (+AR-GARCH resid)  77 features augmented with z
# Report LOSO AUC for both, so the reviewer can see whether accounting for
# temporal dependence changes the supervised performance.
#
...
DAT  <- file.path(ROOT, "revision", "data", "dat_fit.csv")
LAB  <- file.path(ROOT, "revision", "data", "label_tbl.csv")
