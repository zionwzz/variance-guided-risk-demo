# ============================================================================
# 01_sim_add_EF.R   (efficient + faithful: add ONLY Models E and F)
#
# Your A-D are already computed at 100 reps and reported in Supplementary
# Table S1 (A-D reproduce the published 100-rep values).
# There is no need to recompute them. This script computes ONLY the two
# review-requested models, using the IDENTICAL generator, evaluator, tuning, and
# seed scheme as the published A-D run, so the E/F columns are directly
# comparable and can be placed next to the published A-D:
#
#   E  ARXp-GARCHX : metrics_from_fit(sim, TRUE,  lambda_D_beta = 0.01, 0)
#   F  AR-GARCH    : metrics_from_fit(sim, FALSE, 1e3,                  0)
#        (Model F: mean covariates shrunk to ~0 by a large penalty and X excluded
#         from the variance -> covariate-free AR-GARCH; avoids a singular design.)
#
# Runs in PARALLEL (future.apply) so it finishes in ~half an hour, not a day.
#
# HOW TO RUN (from the repository root (variance-guided-risk-demo)):
#   Rscript revision/R/01_sim_add_EF.R
# Quick check first with N_REPS <- 5; then set N_REPS <- 100.
# ============================================================================
...
OUTD <- file.path(ROOT, "revision", "outputs")
