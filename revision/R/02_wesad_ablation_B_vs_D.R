...
#   "one focused WESAD ablation comparing the full penalized ARX-GARCHX model
#    with an otherwise comparable model that excludes covariates from the
#    variance equation, for example Model B versus Model D. Report the AUC and
#    ideally a simple calibration measure for the actual HR-threshold
#    exceedance event."
#
#   Model D (paper's model): ARXp-GARCHXp  -> covariates IN the variance eqn
#   Model B (ablation):      ARXp-GARCH    -> covariates OUT of the variance eqn
#   Both share the SAME penalized mean (lambda_beta = 0.01); they differ ONLY
#   in whether X enters the variance.
#
# Outputs: AUC (stress vs non-stress) and calibration (Brier + reliability +
#          AUC) for the actual exceedance event  y > c_s(0.70).
#
# HOW TO RUN (from VarGuidTS root):
#   Rscript revision/R/02_wesad_ablation_B_vs_D.R
# ============================================================================

ROOT <- normalizePath(".")
EST  <- file.path(ROOT, "R", "temporalVarGuid.r")
DAT  <- file.path(ROOT, "revision", "data", "dat_fit.csv")       # 15 subjects x 77 features
LAB  <- file.path(ROOT, "revision", "data", "label_tbl.csv")     # s, t, label (2 = stress)
