# ============================================================================
# make_revision_figures.R
# Regenerate ALL revision figures in the SAME R/ggplot2 style as the original
# manuscript (boxplots with a white-diamond median, theme_minimal, the original
# 4-hue model colours extended to E/F, NO in-figure title -- titles/captions are
# added in LaTeX). Overwrites the figure files referenced by the manuscript and
# supplement, so nothing else needs to change; just recompile the PDFs.
#
# Produces:
#   revision/outputs/figures/Figure3_sixmodels_bias.pdf      (main-text Figure 3)
#   revision/outputs/figures/Figure3_sixmodels_rmse.pdf
#   revision/outputs/figures/threshold_sensitivity.pdf         (Supplement Fig S1)
#   revision/outputs/figures/modelD_calibration.pdf            (Supplement Fig S2)
#
# HOW TO RUN (from the repository root (variance-guided-risk-demo)):
#   Rscript revision/R/04_make_revision_figures.R
# ============================================================================

ROOT <- normalizePath(".")
suppressMessages({library(ggplot2); library(dplyr); library(tidyr)})

REP  <- file.path(ROOT, "revision", "data")
OUT  <- file.path(ROOT, "revision", "outputs")
SUPA <- file.path(ROOT, "revision", "outputs")
FIGM <- file.path(ROOT, "revision", "outputs", "figures")
FIGS <- file.path(ROOT, "revision", "outputs", "figures")
